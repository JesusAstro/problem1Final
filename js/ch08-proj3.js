...
function CompanyCard(data) {
   ...
   this.revenue = calcRevenue(data);
   this.marketCap50 = calcMarketCap50(data);
   this.marketCap200 = calcMarketCap200(data);
...
function calcRevenue(company) {
   return (company.stats.operatingRevenue - company.stats.costOfRevenue);
}

function calcMarketCap50(company) {
   return (company.stats.sharesOutstanding * company.stats.day50MovingAvg)
 }

function calcMarketCap200(company) {
   return (company.stats.sharesOutstanding * company.stats.day200MovingAvg)
 }

function calcEquity(company) {
   return (company.stats.totalAssets - company.stats.totalLiabilities);
}
